...
public class DijkstrasShortestPathAdjacencyList {
    ...
    public static class Edge {
        ...
        public Edge(int from, int to, double cost, long game) {
            this.from = from;
            this.to = to;
            this.cost = cost;
            this.game = game;
     ...
     */
    public void addEdge(int from, int to, int cost, long game) {
        graph.get(from).add(new Edge(from, to, cost, game));
    }
...
//    }
    ...
    public List<Long> reconstructPath(int start, int end) {
        if (end < 0 || end >= n) throw new IllegalArgumentException("Invalid node index");
        if (start < 0 || start >= n) throw new IllegalArgumentException("Invalid node index");
        double dist = dijkstra(start, end);
        ...
        for (Integer at = end; at != null; at = prev[at]) {
            ...
            neww = at;
            if (at != end) {
                List<Edge> edges = graph.get(neww);
